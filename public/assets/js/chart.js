...
if (ctx != null) {
    console.log(JSON.parse(ctx.getAttribute('data-x')))
    var maxY = JSON.parse(ctx.getAttribute('data-y'))
    var myBarChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: JSON.parse(ctx.getAttribute('data-x')),
            datasets: [{
                label: "Revenue",
                backgroundColor: "#4e73df",
                hoverBackgroundColor: "#2e59d9",
                borderColor: "#4e73df",
                data: JSON.parse(ctx.getAttribute('data-y')),
            }],
        },
        options: {
    ...
    if (ctx2 != null) {
        console.log(JSON.parse(ctx2.getAttribute('data-x')))
        var maxY = JSON.parse(ctx2.getAttribute('data-y'))
